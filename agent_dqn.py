...
class DQNAgent:
    def __init__(self, env, state_shape, action_size, device, args):
        ...
        self.epsilon_decay = (args.epsilon_start -
                              args.epsilon_end) / args.decay_end
        ...
        self.log_interval = args.log_interval if hasattr(
            args, 'log_interval') else 100  # Default logging interval
        self.rewards = []
        self.losses = []
    ...
    def save(self, name):
        torch.save(self.model.state_dict(),
                   os.path.join(self.model_save_path, name))
    ...
    def run_episode(self, batch_size):
        ...
        while not done:
            ...
            if reward > 0:
                reward += 10  # Example of reward shaping
        ...
        if self.epsilon > self.epsilon_min:
            self.epsilon -= self.epsilon_decay  # Fix: should be decrement, not multiplication
    ...
    def train(self, episodes, batch_size, save_interval=100):
        for e in tqdm(range(episodes), desc="Training"):
            ...
            if e % self.log_interval == 0:
                ...
                avg_loss = np.mean(
                    self.losses[-self.log_interval:]) if self.losses else 0
